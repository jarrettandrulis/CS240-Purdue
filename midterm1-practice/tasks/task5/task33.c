 ...
 */
  ...
  */
void censorString(char * str, int startID, int len)
{
	len += startID;
	while(startID < len){
		str[startID++] = 'X';
...
void censorWord(char * str, char * badword)
{
	...
	while(str[counter]!='\0'){
		...
		if(str[counter] == badword[0] && hasWhiteSpace)
		{
			startID = counter;
			while(str[counter] == badword[badwordCounter] && (str[counter] != '\0' && badword[badwordCounter] != '\0') )
				{
					counter++;
					badwordCounter++;
				}
				if((str[counter] == ' ' || str[counter] == '\0')  && badword[badwordCounter] == '\0')
				{
					censorString(str,startID,badwordCounter);
				}
				badwordCounter = 0;
		}
		else
